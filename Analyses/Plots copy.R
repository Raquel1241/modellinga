...
traindat = 
  read.csv("/Users/tyrenkoning/Documents/GitHub/modellinga/Battery_train.csv")
...
ggplot(data = traindat_new, aes(x = Cycle, y = value, group = variable)) +
  ...
  geom_hline(yintercept=0.88, 
             ...
             color = "red", 
             size=0.5) +
  annotate("text", x=1900, y=0.87, 
           label= "Threshold: 0.88", colour = "red", size = 3) +
  theme_bw() + 
...
#Figure 2
for (i in 1:79){
  c = c(seq(sum(!is.na(traindat[i+1])), sum(!is.na(traindat[i+1]))-2057, -1))
  c[c<0] = 0
...
write.csv(
  traindat, "/Users/tyrenkoning/Desktop/University/Modelling-2A/Traindat2.csv",
  row.names=FALSE)
#In between here is where Python data reformatting takes place
traindatlong = read.csv(
  "/Users/tyrenkoning/Desktop/University/Modelling-2A/Traindatlong.csv")
...
#Figure 3
testdat = read.csv(
  "/Users/tyrenkoning/Desktop/University/Modelling-2A/Battery_test.csv")
...
ggplot(data = testdat_new, aes(x = Cycle, y = Capacity, group = Battery)) +
  ...
  geom_hline(yintercept=0.88, 
             ...
             size=0.5) +
  annotate(
    "text", x=1600, y=0.87, label= "Threshold: 0.88", colour = "red", size = 3) +
  theme_bw() + 
  theme(plot.title = element_text(hjust = 0.5, size = 12), 
        panel.border = element_rect(colour = "black", fill=NA, size=0.5), 
        panel.grid.major = element_blank(),
